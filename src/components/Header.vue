...
<header class="bg-white shadow p-4 flex justify-between items-center fixed w-full z-10">
  ...
  <div class="flex-1 mx-2 md:mx-4 relative group">
    <input v-model="searchQuery" type="text" placeholder="Search for project"
        class="w-full sm:px-8 md:px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600">

    <div class="absolute hidden group-focus-within:block w-full bg-white shadow-md rounded-sm flex flex-col gap-2 pb-5 rounded-b-xl mt-2 md:mt-0 sm:w-full md:w-10 lg:w-96 xl:w-120">
        <div class="hover:bg-slate-300 h-10 px-4" v-if="searchQuery !== ''" v-for="project in filteredProjects"
            :key="project.id">{{ project.name }}</div>
    </div>
</div>

   <div class="flex items-center sm:space-x-2 md:space-x-4 px-2 md:px-4 lg:px-8 xl:px-20">
  <button class="p-1 md:p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-600">
    ...
    <svg class="h-4 w-4 md:h-6 md:w-6 sm:h-5 sm:w-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"
