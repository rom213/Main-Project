<script setup lang="ts">
import { useProjectStore } from '../store/projects.ts'
import { useNavigationStore } from "../store/navigation";
import { computed, ref, onMounted, onBeforeUnmount } from 'vue'
import { Project } from '../types/projectTypes.ts';
import { useRouter } from 'vue-router';

const projectStore = useProjectStore()
const searchQuery = ref<string | null>(null)
const isDropdownVisible = ref(false)
const navigationStore = useNavigationStore()
const router = useRouter()

const filteredProjects = computed(() => {
  if (searchQuery.value) {
    return projectStore.searchProjectsByName(searchQuery.value)
  }
  return [] // Retornar lista vacía si no hay búsqueda
})

const redirectProject = (project: Project) => {
  navigationStore.project = project;
  router.push('/projects')
  isDropdownVisible.value = false; // Ocultar la lista
}

// Manejar clics fuera de la lista
const handleClickOutside = (event: Event) => {
  const target = event.target as HTMLElement;
  if (!target.closest('.dropdown-container') && !target.closest('.search-input')) {
    isDropdownVisible.value = false;
  }
}

onMounted(() => {
  document.addEventListener('click', handleClickOutside);
})

onBeforeUnmount(() => {
  document.removeEventListener('click', handleClickOutside);
})
</script>

<template>
<header class="bg-white shadow p-2 flex flex-wrap justify-between items-center fixed w-full z-10">
  <!-- Logo -->
  <div class="flex items-center space-x-2 md:space-x-4 lg:space-x-6 xl:space-x-8 md:px-4 lg:px-8 xl:px-11 ">
    <img src="https://cdn.pixabay.com/photo/2017/03/16/21/18/logo-2150297_640.png" alt="Logo" class="h-8 w-8">
    <span class="text-xl font-semibold sm:text-lg md:text-xl lg:text-2xl xl:text-3xl">Main Project</span>
  </div>

  <!-- Search Bar -->
  <div class="flex-1 mx-2 md:mx-4 relative group w-full md:w-full">
      <input v-model="searchQuery" @focus="isDropdownVisible = true" type="text" placeholder="Search for project"
        class="search-input w-full lg:min-w-[100px] sm:px-8 md:px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-600">
      
      <div v-if="isDropdownVisible" class="dropdown-container absolute w-[200px] bg-white shadow-md rounded-sm flex flex-col gap-2 pb-5 rounded-b-xl mt-2 md:mt-0 sm:w-full md:w-10 lg:w-96 xl:w-120">
        <div class="hover:bg-slate-300 min-h-8 px-4 cursor-pointer object-fill overflow-hidden" v-for="project in filteredProjects"
          :key="project.id" @click="redirectProject(project)">{{ project.name }}</div>
      </div>
    </div>

  <!-- Icons -->
  <div class="flex items-center space-x-2 md:space-x-4 px-2 md:px-4 lg:px-8 xl:px-20">
    <button class="p-1 md:p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-600">
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
        class="h-4 w-4 md:h-6 md:w-6 sm:h-5 sm:w-5">
        <path stroke-linecap="round" stroke-linejoin="round"
          d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" />
      </svg>
    </button>
    <button class="md:p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-600">
      <svg class="h-4 w-4 md:h-6 md:w-6 sm:h-5 sm:w-5 text-gray-600" fill="none" stroke="currentColor"
        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8h18M3 12h18M3 16h18"></path>
      </svg>
    </button>
    <button class="md:p-2 rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-purple-600">
      <svg class="h-4 w-4 md:h-6 md:w-6 sm:h-5 sm:w-5 text-gray-600" fill="none" stroke="currentColor"
        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
      </svg>
    </button>
    <div class="flex items-center space-x-1">
        <img
          src="data:image/jpeg;base64,"
          alt="User" class="h-10 w-10 rounded-full">
        <div class="text-sm lg:text-base xl:text-lg 2xl:text-xl sm:text-xs sm:flex sm:flex-col sm:items-start">
          <div class="font-semibold">Romario Ariza</div>
          <div class="text-gray-500">SAN, Colombia</div>
        </div>
      </div>
  </div>
</header>
</template>
