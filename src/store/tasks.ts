...
import { ProjectStatus, TaskImportance, TaskStatus } from '../enums/status';
...
export const useTaskStore = defineStore('tasks', () => {
  ...
  const allTaskByStatus = (status: TaskStatus) => {
    return tasks.value.filter(task => {
      const project = projectStore.projects.find(project => project.id === task.idProject);
  
      if (!project || project.status === ProjectStatus.Inactive
      ) {
        return false;
      }
